...
function generateMockData(period: PeriodTab, groupBy: GroupBy): CommissionData {
  const now = new Date();
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const amount = parseFloat((Math.random() * 2500 + 500).toFixed(2));
...
export default function FinanceCommissionScreen() {
  ...
  const loadData = useCallback(() => {
    setLoading(true);
    // Simulate async data fetch (replace with tRPC call when backend endpoint is ready)
    setTimeout(() => {
      setData(generateMockData(period, groupBy));
      setLoading(false);
    }, 600);
  }, [period, groupBy]);
