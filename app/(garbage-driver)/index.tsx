 ...
 *
 * devMode: generates mock pickup data when none exist.
...
function generateMockPickups(driverId: string, zoneId: string): Pickup[] {
  const addresses = [
    { name: "Banda Family", address: "Plot 45, Kabulonga Road, Lusaka", lat: -15.3875, lng: 28.3228 },
    { name: "Mwale Household", address: "House 12, Ibex Hill, Lusaka", lat: -15.4012, lng: 28.3456 },
    { name: "Phiri Residence", address: "Stand 78, Chelstone, Lusaka", lat: -15.4234, lng: 28.3012 },
    { name: "Tembo Family", address: "Plot 23, Woodlands, Lusaka", lat: -15.3756, lng: 28.3567 },
    { name: "Mutale Home", address: "House 5, Roma, Lusaka", lat: -15.4089, lng: 28.2987 },
  ];
  const now = new Date();
  return addresses.map((a, i) => ({
    ...
    address: a.address,
    zoneId,
    status: (i === 0 ? "in_progress" : i < 3 ? "assigned" : "completed") as PickupStatus,
    assignedDriverId: driverId,
    scheduledDate: new Date(now.getTime() + i * 3600000).toISOString(),
    completedAt: i >= 3 ? new Date(now.getTime() - 3600000).toISOString() : undefined,
    notes: i === 1 ? "Large bin — bring extra bags" : undefined,
    subscriptionStatus: (i === 2 ? "overdue" : "active") as "active" | "overdue",
    latitude: a.lat,
    longitude: a.lng,
    contactPhone: "+260960" + String(100000 + i * 11111),
    contactName: a.name.split(" ")[0],
  }));
}
    ...
    try {
      ...
      let filtered = all.filter(
        (p) =>
          ...
          (driverZoneId ? p.zoneId === driverZoneId : true)
      );

      if (filtered.length === 0 && isDevMode) {
        const mockZoneId = driverZoneId || "dev_zone_1";
        const mocks = generateMockPickups(user.id, mockZoneId);
        const merged = [...all.filter((p) => !p.id.startsWith("mock_pickup_")), ...mocks];
