...
export default function LinkAccountScreen() {
  ...
  // Mock existing linked account - will be fetched from database
  ...
  const handleLinkAccount = async () => {
    ...
    try {
      // TODO: Call API to link account
      // await api.wallet.linkAccount({ provider: selectedProvider, phoneNumber, pin: withdrawalPin });
      ...
      Alert.alert(
        ...
        `Your ${providers.find((p) => p.id === selectedProvider)?.name} account has been linked. You can now withdraw funds to ${phoneNumber}.`,
        [
          {
            text: "OK",
            onPress: () => router.back(),
          ...
          <View className="px-6 mb-6">
            <View className="bg-primary/10 border border-primary rounded-2xl p-4">
              <View className="flex-row items-center justify-between mb-3">
                ...
                <TouchableOpacity onPress={handleUnlinkAccount}>
                  <Text className="text-error font-medium">Unlink</Text>
                </TouchableOpacity>
              </View>
