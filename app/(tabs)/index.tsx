...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const pollDriverLocation = async () => {
      try {
        // Find the most recent active pickup for this customer
        const raw = await AsyncStorage.getItem('@ltc_pickups');
        const allPickups: any[] = raw ? JSON.parse(raw) : [];
  ...
  return (
    <ScreenContainer>
      ...
      >
        ...
        {isCustomer && (
          <View className="px-6 mb-6">
            {/* Dev mode bypass banner — hidden when a real pending/approved request exists */}
            {!APP_CONFIG.requireSubscriptionForPickup && !hasPendingApproval && !hasApprovedAwaitingActivation && (
              <View className="bg-warning/15 rounded-2xl p-4 mb-3 flex-row items-center border border-warning/30">
                <MaterialIcons name="developer-mode" size={20} color="#F59E0B" />
                <View className="ml-3 flex-1">
                  <Text className="text-warning font-semibold text-sm">
                    Development Mode
                  </Text>
                  <Text className="text-warning/80 text-xs mt-0.5">
                    Subscription pending approval. Pickups are enabled for testing.
                  </Text>
                </View>
              </View>
            )}
