 ...
 *
 * Data source: provider_wallets + payment_transactions (via tRPC)
 */
...
const WITHDRAWAL_METHODS: { value: WithdrawalMethod; label: string; hint: string }[] = [
  { value: "mtn_momo", label: "MTN MoMo", hint: "Enter MTN phone number (e.g. 0971234567)" },
  { value: "airtel_money", label: "Airtel Money", hint: "Enter Airtel phone number" },
  { value: "zamtel_money", label: "Zamtel Money", hint: "Enter Zamtel phone number" },
  { value: "bank_transfer", label: "Bank Transfer", hint: "Enter bank account number" },
];

// ─── Mock Data (used when tRPC endpoints are not yet wired) ───────────────────

const MOCK_WALLET: ProviderWalletData = {
  availableBalance: "2340.00",
  totalEarned: "8750.50",
  totalWithdrawn: "6410.50",
  pendingBalance: "180.00",
};

const MOCK_TRANSACTIONS: TransactionItem[] = [
  { id: 1, serviceType: "garbage", amountTotal: "250.00", providerAmount: "225.00", platformCommission: "25.00", status: "completed", paymentMethod: "mtn_momo", createdAt: "2026-03-01T10:00:00Z", referenceId: "ref-001" },
  { id: 2, serviceType: "carrier", amountTotal: "500.00", providerAmount: "450.00", platformCommission: "50.00", status: "completed", paymentMethod: "mtn_momo", createdAt: "2026-02-28T14:30:00Z", referenceId: "ref-002" },
  { id: 3, serviceType: "garbage", amountTotal: "180.00", providerAmount: "162.00", platformCommission: "18.00", status: "pending", paymentMethod: "mtn_momo", createdAt: "2026-02-27T09:15:00Z", referenceId: "ref-003" },
  { id: 4, serviceType: "carrier", amountTotal: "320.00", providerAmount: "288.00", platformCommission: "32.00", status: "completed", paymentMethod: "bank_transfer", createdAt: "2026-02-25T16:45:00Z", referenceId: "ref-004" },
  { id: 5, serviceType: "garbage", amountTotal: "150.00", providerAmount: "135.00", platformCommission: "15.00", status: "failed", paymentMethod: "mtn_momo", createdAt: "2026-02-24T11:00:00Z", referenceId: "ref-005" },
];
...
export default function ProviderEarningsScreen() {
  ...
  // Use mock data — replace with real tRPC query when provider auth is wired
  const wallet: ProviderWalletData = MOCK_WALLET;
  const transactions: TransactionItem[] = MOCK_TRANSACTIONS;
  const loading = false;
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    if (activeTab === "completed") return t.status === "completed" || t.status === "released";
  ...
  const handleWithdrawSubmit = () => {
    ...
    if (amount > available) {
      Alert.alert("Insufficient Balance", `Maximum available: ${formatZMW(available)}`);
      return;
    }
    ...
    setSubmitting(true);
    // Simulate submission — replace with trpc.paymentService.withdraw.mutate(...)
    setTimeout(() => {
      setSubmitting(false);
      ...
      Alert.alert(
        ...
        `Your withdrawal of ${formatZMW(amount)} via ${WITHDRAWAL_METHODS.find((m) => m.value === withdrawMethod)?.label} has been submitted for admin approval.`,
      );
    }, 1500);
  };
      ...
      >
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: colors.muted }]}>Loading earnings...</Text>
          </View>
        ) : (
          <>
            {/* Balance Cards */}
            <View style={styles.cardsRow}>
              <View style={[styles.balanceCard, styles.balanceCardMain, { backgroundColor: colors.primary }]}>
                ...
                <Text style={styles.balanceAmountLarge}>{formatZMW(wallet.availableBalance)}</Text>
                <TouchableOpacity
            ...
            <View style={styles.statsRow}>
              <View style={[styles.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                ...
                <Text style={[styles.statValue, { color: colors.success }]}>{formatZMW(wallet.totalEarned)}</Text>
              </View>
              <View style={[styles.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.statLabel, { color: colors.muted }]}>Total Withdrawn</Text>
                <Text style={[styles.statValue, { color: colors.foreground }]}>{formatZMW(wallet.totalWithdrawn)}</Text>
              </View>
              <View style={[styles.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.statLabel, { color: colors.muted }]}>Pending</Text>
                <Text style={[styles.statValue, { color: colors.warning }]}>{formatZMW(wallet.pendingBalance)}</Text>
              </View>
            </View>
            ...
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.txnCount, { color: colors.muted }]}>{filteredTransactions.length} records</Text>
            </View>
                ...
                >
                  <View style={styles.txnTop}>
                    <View style={styles.txnLeft}>
                      <Text style={[styles.txnService, { color: colors.foreground }]}>
                        {txn.serviceType === "garbage" ? "🗑 Garbage Collection" : "🚚 Carrier Service"}
                      </Text>
                      <Text style={[styles.txnDate, { color: colors.muted }]}>{formatDate(txn.createdAt)}</Text>
                      {txn.referenceId && (
                        <Text style={[styles.txnRef, { color: colors.muted }]}>Ref: {txn.referenceId}</Text>
                      )}
                    </View>
                    <View style={styles.txnRight}>
                      <Text style={[styles.txnAmount, { color: colors.success }]}>
                        +{formatZMW(txn.providerAmount)}
                      </Text>
                      <View style={[styles.statusBadge, { backgroundColor: statusColor(txn.status, colors) + "22" }]}>
                        <Text style={[styles.statusText, { color: statusColor(txn.status, colors) }]}>
                  ...
                  <View style={styles.txnBottom}>
                    <Text style={[styles.txnDetail, { color: colors.muted }]}>
                      Total: {formatZMW(txn.amountTotal)} · Commission: {formatZMW(txn.platformCommission)}
                    </Text>
                    <Text style={[styles.txnMethod, { color: colors.muted }]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { backgroundColor: colors.background }]}>
            ...
            <Text style={[styles.modalSubtitle, { color: colors.muted }]}>
              Available: {formatZMW(wallet.availableBalance)}
            </Text>
            ...
            <View style={[styles.commissionNote, { backgroundColor: colors.surface }]}>
              <Text style={[styles.commissionNoteText, { color: colors.muted }]}>
                Withdrawal requests are reviewed by an admin before processing. Approved payouts are sent via MTN MoMo or your selected method.
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  loadingContainer: { flex: 1, alignItems: "center", justifyContent: "center", paddingTop: _rs.sp(80) },
  loadingText: { marginTop: _rs.sp(12), fontSize: _rs.fs(14) },
