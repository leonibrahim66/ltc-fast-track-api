...
const calculateDistanceKm = async (origin: string, destination: string): Promise<number | null> => {
  try {
    // Simulate API call delay
    await new Promise((resolve) => setTimeout(resolve, 800));
    
    // Simulate distance calculation based on string length (for demo purposes)
    // In production, use: https://maps.googleapis.com/maps/api/distancematrix/json
    const mockDistance = Math.abs(origin.length - destination.length) * 5 + Math.random() * 50 + 10;
...
export default function BookCarrierScreen() {
  ...
  const validateStep2 = () => {
    ...
    if (calculatingDistance) { Alert.alert("Please Wait", "Calculating distance..."); return false; }
    if (!calculatedDistance) { Alert.alert("Error", "Unable to calculate distance. Please check your locations."); return false; }
    return true;
  };
        ...
        <View className="bg-surface rounded-xl border border-border p-4">
          <View className="flex-row items-center justify-between">
            <View className="flex-1">
              <Text className="text-xs text-muted mb-1">CALCULATED DISTANCE</Text>
              {calculatingDistance ? (
                <View className="flex-row items-center">
                  <ActivityIndicator size="small" color="#22C55E" />
                  <Text className="text-sm text-muted ml-2">Calculating...</Text>
                </View>
              ) : calculatedDistance ? (
                <Text className="text-2xl font-bold text-foreground">{calculatedDistance} km</Text>
              ) : null}
            </View>
